...
def test_get_with_retry_success():
    with patch('requests.get') as mock_get, \
            ...
            patch('app.components.request_handler.log') as mock_log:
...
def test_get_with_retry_http_429():
    with patch('requests.get') as mock_get, \
            patch('time.sleep') as mock_sleep, \
            patch('app.components.request_handler.log') as mock_log:
        ...
        assert any(f"Too many requests for http://example.com, retrying in " in str(call) for call in
                   mock_log.info.call_args_list)
        assert any(f"Sleeping for " in str(call) for call in mock_log.info.call_args_list)
        assert mock_sleep.call_count == 2


def test_get_with_retry_request_exception():
    with patch('requests.get') as mock_get, \
            patch('time.sleep') as mock_sleep, \
            patch('app.components.request_handler.log') as mock_log:
        ...
        assert mock_get.call_count == 2
        assert any(
            f"Request failed for http://example.com due to " in str(call) for call in mock_log.info.call_args_list)
        assert any(f"Sleeping for " in str(call) for call in mock_log.info.call_args_list)
        assert mock_sleep.call_count == 2


def test_get_with_retry_max_retries_exceeded():
    with patch('requests.get') as mock_get, \
            patch('time.sleep') as mock_sleep, \
            patch('app.components.request_handler.log') as mock_log:
